...
class LoginService {
    private let _base = NSLocalizedString("main", comment: "")
    private let _clientId = NSLocalizedString("clientId", comment: "")
    private let _clientSecret = NSLocalizedString("clientSecret", comment: "")
    private let _redirect = "github-pr://login"
    private let _decoder = JSONDecoder()
    private var _session: ASWebAuthenticationSession?
    
    func getCode(_ session: ASWebAuthenticationPresentationContextProviding, onComplete: ((_ code: String?) -> Void)?) {
        let url = URL(string: "\(self._base)/login/oauth/authorize?client_id=\(self._clientId)&redirect_uri=\(self._redirect)")
    ...
    func getToken(_ code: String, onComplete: ((_ token: String?) -> Void)?) {
        let _ = HttpClient.get(
            "\(self._base)/login/oauth/access_token?client_id=\(self._clientId)&client_secret=\(self._clientSecret)&code=\(code)&redirect_uri=\(self._redirect)"
        ) {
